...
class Driver(object):
    def __init__(self, environment, reweight, ensemble):
        ...
        self.algorithm = MGAIL(environment=self.env, reweight=self.reweight, ensemble=self.ensemble)
    ...
    def train_forward_model(self):
        ...
        states_e_, actions_e, _, states_e = self.algorithm.er_expert.sample()[:4]

        if self.reweight:
            fetches = [alg.forward_model.ex_weights_]
            feed_dict = {alg.states_: states_, alg.states: states, alg.actions: actions,
                         alg.states_e_: states_e_, alg.states_e: states_e, alg.actions_e: actions_e,
                         alg.do_keep_prob: self.env.do_keep_prob}
            ex_weights = self.sess.run(fetches, feed_dict)
            ex_weights = ex_weights[0]

            fetches = [alg.forward_model.train_op, alg.forward_model.loss_c]
            feed_dict = {alg.states_: states_, alg.states: states, alg.actions: actions,
                         alg.ex_wts_: ex_weights, alg.do_keep_prob: self.env.do_keep_prob}
        else:
            fetches = [alg.forward_model.train_op, alg.forward_model.loss_c]
            feed_dict = {alg.states_: states_, alg.states: states, alg.actions: actions,
                         alg.do_keep_prob: self.env.do_keep_prob}
    ...
    def predict_forward_model(self, path):
        alg = self.algorithm
        states_, actions, _, states = self.algorithm.er_agent.sample()[:4]
        fetches = [alg.forward_model.prediction]
        feed_dict = {alg.states_: states_, alg.states: states, alg.actions: actions,
                     alg.do_keep_prob: self.env.do_keep_prob}
        run_vals = self.sess.run(fetches, feed_dict)
        next_states, gru_states = run_vals[0]
        result = np.append(states, next_states, axis=0)
        
        filename = '{}_{}.txt'.format(path, self.itr)
